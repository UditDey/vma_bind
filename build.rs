...
fn main() {
    ...
    // Set includes
    let vma_include_path = Path::new("vendor").join("VulkanMemoryAllocator").join("include");
    let vk_header_include_path = Path::new("vendor").join("Vulkan-Headers").join("include");
    ...
    // Set source files
    build.file(Path::new("vendor").join("vma.cpp"));
    ...
    let binding_path = PathBuf::from(env::var("OUT_DIR").unwrap()).join("vma_ffi.rs");
